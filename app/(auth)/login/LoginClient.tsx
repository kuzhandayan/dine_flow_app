...
const LOGOUT_REASON_MESSAGES: Record<string, { title: string; message: string }> = {
  ...
  AccountDisabled: {
    ...
    message: 'Your staff account was deactivated. Contact your restaurant owner.',
  },
  TenantSuspended: {
    title: 'Restaurant account suspended',
    message: 'Contact the platform admin to restore access.',
  },
}
...
function LoginForm(): React.JSX.Element {
  ...
  const shownReasonToast = useRef(false)
  const [showPassword, setShowPassword] = useState(false)
  const [serverError, setServerError] = useState<string | null>(null)
  const [isAccessError, setIsAccessError] = useState(false)

  // Explain to the user why they landed back on the login page (forced sign-out)
  useEffect(() => {
    if (shownReasonToast.current) return
    const reason = searchParams.get('reason')
    if (!reason) return
    shownReasonToast.current = true
    const copy = LOGOUT_REASON_MESSAGES[reason]
    if (copy) toast.warning(copy.title, copy.message)
